...
class DQN:
    ...
    def learn(self):
        ...
        if self.buffer.check_state():
            ...
            #save_batch = np.array(batch)
            #np.save('/data0/chenx/imdb_QO/Models/Model_Clip/experience/{}_save_batch.npy'.format(self.cur_epoch), save_batch)

            #count = sum([i[4] for i in batch])/self.batch_size
            #self.first_step_prob.append((count, self.cur_epoch))
            #with open('/data0/chenx/imdb_QO/Models/Model_Clip/first_step_prob.json', 'w') as f:
            #    json.dump(self.first_step_prob, f)
