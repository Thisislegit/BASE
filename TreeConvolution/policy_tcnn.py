...
def connectation(query, plan, device):
    b = torch.tensor(plan[0], device=device, dtype=torch.float32)
    a = torch.cat((query, b))
    goal = [a]
    if len(plan) > 1 and len(plan) < 4:
        left = connectation(query, plan[1], device)
        right = connectation(query, plan[2], device)
...
class QueryEncoder(nn.Module):
    ...
    def forward(self, goal):
        ...
        if isinstance(goal, tuple):
            goal, available_table, start = goal
            return (self.forward(goal), available_table, start)
        ...
        if len(goal) == 2 and isinstance(goal[1], tuple):
            ...
            a = []
            a = a + connectation(x, y, self.local_device)
