...
def _flatten(root, transformer, left_child, right_child, encode):
    ...
    def recurse(x, encode):
        if _is_leaf(x, left_child, right_child):
            accum.append(transformer(x, encode))
            return
        # 改为后续遍历
...
def _tree_conv_indexes(root, left_child, right_child):
    ...
    return torch.tensor(np.array(list(recurse(index_tree))).flatten().reshape(-1, 1), dtype=torch.int64)
...
def _pad_and_combine(x):
    ...
    zero_pos = []
    for arr in x:
        padded = torch.zeros((max_first_dim, second_dim))
...
def prepare_trees(trees, transformer, left_child, right_child, encode):
    flat_trees = [_flatten(x, transformer, left_child, right_child, encode[batch]) for batch, x in enumerate(trees)]
    flat_trees, zero_pos = _pad_and_combine(flat_trees)
