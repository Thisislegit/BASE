...
def _flatten(root, transformer, left_child, right_child, encode, local_device):
    ...
    def recurse(x, encode):
        if _is_leaf(x, left_child, right_child):
            accum.append(transformer(x, encode, local_device))
            return

        accum.append(transformer(x, encode, local_device))
        recurse(left_child(x), encode)
        recurse(right_child(x), encode)

    recurse(root, encode)
    try:
        # accum = [torch.zeros(accum[0].shape)] + accum
        pass
    except:
        raise TreeConvolutionError(
    ...
    return torch.cat([i.unsqueeze(0) for i in accum], dim=0)
...
def _tree_conv_indexes(root, left_child, right_child, local_device):
    """
    ...
    return torch.tensor(np.array(list(recurse(index_tree))).flatten().reshape(-1, 1), dtype=torch.int64, device=local_device)


def _pad_and_combine(x, local_device):
    assert len(x) >= 1
    ...
    zero_pos = []
    for arr in x:
        padded = torch.zeros((max_first_dim, second_dim), device=local_device)
        padded[0:arr.shape[0]] = arr
        vecs.append(padded)
        zero_pos.append(arr.shape[0])
    return torch.cat([i.unsqueeze(0) for i in vecs], dim=0), zero_pos


def prepare_trees(trees, transformer, left_child, right_child, encode, local_device):
    if encode is not None:
        flat_trees = [_flatten(x, transformer, left_child, right_child, encode[batch], local_device=local_device) for batch, x in enumerate(trees)]
    else:
        flat_trees = [_flatten(x, transformer, left_child, right_child, encode, local_device=local_device) for
                      batch, x in enumerate(trees)]
    flat_trees, zero_pos = _pad_and_combine(flat_trees, local_device)
    # flat_trees = torch.Tensor(flat_trees)
    # print(flat_trees.shape, flat_trees.dtype)

    # flat trees is now batch x max tree nodes x channels
    flat_trees = flat_trees.transpose(1, 2)
    # print(flat_trees.shape)

    indexes = [_tree_conv_indexes(x, left_child, right_child, local_device=local_device) for x in trees]
    indexes, _ = _pad_and_combine(indexes, local_device)
    indexes = indexes.type(torch.int64)
    zero_pos = torch.tensor(zero_pos, dtype=torch.int64, device=local_device)
    # indexes = torch.Tensor(indexes).long()
    # print(indexes.shape, len(zero_pos))
    return (flat_trees, indexes, zero_pos)