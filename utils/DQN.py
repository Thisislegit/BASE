...
class DQN:
    ...
    def learn(self):
        ...
        if self.buffer.check_state():
            ...
            nn.utils.clip_grad_norm_(self.net.parameters(), max_norm=10)
...
class Distibut_DQN:
    def __init__(
        ...
        # self.target_net_model = target_net_model
        # self.net_model = net_model
        self.target_net = target_net_model
        # self.target_device = next(self.target_net.parameters()).device
        self.net = net_model
        # self.net_device = next(self.net.parameters()).device
        self.count = 0
        self.learn_step_counter = 0
        self.target_replace_iter = 20
        self.sync_batch_size = sync_batch_size
    ...
    def learn(self, local_device):
        self.learn_step_counter += 1

        if self.buffer.check_state():
            batch, importance, indices = self.buffer.sample(self.batch_size,
                                                            (self.learn_step_counter % 2 == 0) * 0.7)
            self.count += 1
            self.update_net()
            ...
            self.net.train()
            q_eval = self.net(now_states)
            q_eval = q_eval.to(local_device)
            next_v = self.target_net(next_states).detach()
            for idx in zero_batches:
                next_v[idx] = 0
            q_target = torch.tensor([batch[i][0] for i in range(len(batch))], device=local_device).unsqueeze(1) + next_v * self.gamma
            ...
            loss = self.compute_loss(q_eval, q_target)
            importance = torch.tensor(importance ** (1 - self.epsilon_start), device=local_device).unsqueeze(1)
            loss = torch.mean(importance * loss)

            loss.backward()

            # Important: add grad_clip
            nn.utils.clip_grad_norm_(self.net.parameters(), max_norm=20)
...
class DQN_latency(DQN):
    def __init__(
            self, target_net_model, net_model,wc=1,wl=1,
            capacity=50, epsilon_start=1, epsilon_end=0.1, epsilon_decay=0.85, batch_size=20, sync_batch_size=30, path='./Models/now.pth'):
        # self.buffer = ExperienceBuffer(capacity)
        self.buffer = PrioritizedReplayBuffer(capacity)
        self.epsilon_start = epsilon_start
    ...
    def learn(self):
        ...
        # todo：第三版
        if self.buffer.check_state():
            batch, importance, indices = self.buffer.sample(
                self.batch_size, (self.learn_step_counter % 2 == 0)*0.7)
            self.count += 1
            self.update_net()

            zero = [[0]*len(batch[0][1][0]),
                    (tuple([0] * (len(all_tables) * 2 + 3)),)]
            zero_batches = []
            # now state
            now_states = [i[1] for i in batch]
            next_states = []
            ...
            now_values,_ = self.net(all_action)
            ...
            for idx, i in enumerate(batch):
                if i[2]:
                    ...
                    min_idx = torch.argmin(
                        now_values[cur_idx:cur_idx+length], dim=0).item()
                    next_states.append(i[2][min_idx])
                    cur_idx += length
                else:
                    assert i[3] == 0
            ...
            assert len(now_states) == len(next_states)
            self.net.train()
            latency_eval,q_eval=self.net(now_states)

            next_v_c,next_v_l = self.target_net(next_states)
            next_v_c=next_v_c.detach()
            next_v_l=next_v_l.detach()

            for idx in zero_batches:
                next_v_c[idx] = 0
            q_target = torch.tensor([batch[i][0] for i in range(len(batch))]).unsqueeze(
                1) + next_v_c * self.gamma +next_v_l * self.gamma

            self.optimizer.zero_grad()
            error = torch.abs(self.wc*q_eval + self.wl*latency_eval -
                              q_target).cpu().data.numpy().reshape(-1).tolist()
            # update priority
            self.buffer.set_priorities(indices, error)
            # 这里如果WC，WL不全为1，需要在计算q_target时，将cost与latency也乘以对应的权重
            loss = self.compute_loss(self.wc*q_eval + self.wl*latency_eval, q_target)
            importance = torch.tensor(
                importance**(1-self.epsilon_start)).unsqueeze(1)
            loss = torch.mean(importance*loss)
