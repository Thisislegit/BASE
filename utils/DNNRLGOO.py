...
class DNNRLGOO(object):
    def __init__(self, model, optimizer, table, query_encode, selectivity, join_table, file_name, table_alias, target_model, test=False):
        self.target_net = target_model
        # self.target_device = next(self.target_net.parameters()).device
        self.value_net = model
        # self.value_device = next(self.value_net.parameters()).device
    ...
    def init_state(self):
        ...
        if ret < self.random_num:
            ...
            join_type_index = random.choice([0, 2])
            ...
            if not self.test:
                now_value = self.value_net(all_actions)
            else:
                now_value = self.target_net(all_actions)
    ...
    def choose_action(self):
        ...
        if ret < self.random_num:
            ...
            if not self.test:
                ...
                now_values = self.target_net(all_actions)
