...
def travesal_SQL(sql):
    def add_val(SQL_encode, t, att, val):
        assert val >= 0
        if SQL_encode[4].get('.'.join([SQL_encode[3][t], att])):
            count = 0
            for name in SQL_encode[3].values():
                if name == SQL_encode[3][t]:
                    count += 1
            if count > 1:
                SQL_encode[4]['.'.join([SQL_encode[3][t], att])] = SQL_encode[4]['.'.join([SQL_encode[3][t], att])] + val
            else:
                SQL_encode[4]['.'.join([SQL_encode[3][t], att])] = SQL_encode[4]['.'.join([SQL_encode[3][t], att])] * val
        else:
            SQL_encode[4]['.'.join([SQL_encode[3][t], att])] = val
    ...
    for token in sqlparse.parse(sql)[0].tokens:
        if isinstance(token, Where):
            lst = [one.value for one in token.tokens if one.ttype is not Whitespace]
            for idx, one in enumerate(lst):
                ...
                if 'IS' == one:
                    if 'NOT ' not in lst[idx+1]:
                        ...
                        t, att = result[0].split('.')
                        add_val(SQL_encode, t, att, get_equal_selectivity((att, '=', result[2]), 0, SQL_encode[3][t])[2])
                    else:
                        result = lst[idx - 1:idx + 2]
                        t, att = result[0].split('.')
                        add_val(SQL_encode, t, att, get_equal_selectivity((att, '=', result[2][4:]), 1, SQL_encode[3][t])[2])
                if 'ANY' == one:
                    val = 0
                    result = lst[idx-2:idx+2]
                    pattern = re.compile(r'[\{,]\"([^\"]*?)\"(?=[\},])|[\{,](.*?)(?=[\},])')
                    query = [a if a else b for a, b in pattern.findall(result[-1])]
                    t, att = result[0].split('.')
                    for one in query:
                        val += get_equal_selectivity((att, '=', one), 0, SQL_encode[3][t])[2]
        ...
        if isinstance(token, Where):
            for one in token.tokens:
                ...
                if isinstance(one, Comparison):
                    if (' =' in one.value or ' !=' in one.value or ' <>' in one.value) and (' >=' not in one.value) and (' <=' not in one.value):
                        ...
                        if ("'" not in b) and ("." in b):
                            ...
                            if "'" in b:
                                b = b.replace("'", "")
                            t, att = a.split('.')
                            if ' =' in one.value and ' !=' not in one.value:
                                add_val(SQL_encode, t, att,
                                        get_equal_selectivity((att, '=', b),0, SQL_encode[3][t])[2])
                            else:
                                add_val(SQL_encode, t, att,
                                        get_equal_selectivity((att, '=', b), 1, SQL_encode[3][t])[2])
                        ...
                        if "'" in b:
                            b = b.replace("'", "")
                        t, att = a.split('.')
                        op = re.findall('>=|<=|<|>', one.value)
                        assert len(op) == 1
                        add_val(SQL_encode, t, att, get_scalar_selectivity((att, op[0], b), SQL_encode[3][t])[2])
                        ...
                        val = _get_selectivity(t.replace('(', '').strip(), one.value)
                        add_val(SQL_encode, t, att, val)
                    ...
                    for idx, one in enumerate(lst):
                        if (idx == 0):
                            val, relation = process(one, SQL_encode)
                            add_val(SQL_encode, relation[0], relation[1], val)
                        else:
